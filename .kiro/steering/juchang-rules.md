...
- **Mandate**:
  ...
  - **Type Inference**: 使用类型守卫替代 `as` 断言，使用解构排除敏感字段（详见 `typescript-type-inference.md`）
...
- **Mandate**:
  ...
  - **Type Inference**: Page 使用泛型 `Page<XxxPageData, WechatMiniprogram.Page.CustomOption>`，Storage 使用默认值替代断言（详见 `typescript-type-inference.md`）
...
- **Mandate**:
  ...
  - **Type Inference**: 使用 TanStack Router 的 `validateSearch` 类型推导，避免 URL 参数断言（详见 `typescript-type-inference.md`）
