...
export function TransactionsTable({ data }: DataTableProps) {
  ...
  const table = useReactTable({
    ...
    globalFilterFn: (row, _columnId, filterValue) => {
      ...
      const userId = String(row.getValue('userId')).toLowerCase()
      const description = String(row.getValue('description') || '').toLowerCase()
      const searchValue = String(filterValue).toLowerCase()

      return id.includes(searchValue) || userId.includes(searchValue) || description.includes(searchValue)
    },
  ...
  return (
    ...
    >
      <DataTableToolbar
        table={table}
        searchPlaceholder='按交易ID、用户ID或描述搜索...'
        filters={[
          {
            columnId: 'status',
            title: '状态',
