...
### 1.2 v3.2 架构：Chat-First + Generative UI
...
**核心交互哲学**：
1. **首页即对话**：App 打开不再是地图，而是一个无限滚动的 Chat View
...
| 页面 | 路径 | 类型 |
|-----|------|------|
| 首页 | `pages/home/index` | 主包 |
| 个人中心 | `pages/profile/index` | 主包 |
| 消息中心 | `pages/message/index` | 主包 |
| 活动群聊 | `pages/chat/index` | 主包 |
| 活动详情 | `subpackages/activity/detail/index` | 分包 |
| 活动确认 | `subpackages/activity/confirm/index` | 分包 |
| 活动列表 | `subpackages/activity/list/index` | 分包 |
| 地图选点 | `subpackages/activity/map-picker/index` | 分包 |
| **沉浸式地图** | `subpackages/activity/explore/index` | **分包 (新增)** |
...
**创建场景**：
```
首页(1) → 确认发布(2) → 分享成功 = 2 层 ✅
```

**探索场景**：
```
首页(1) → 沉浸式地图(2) → 活动详情(3) = 3 层 ✅
```
