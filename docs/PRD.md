...
> **版本**：v4.2 (Agent-First + Broker Mode)
> **更新日期**：2026-01
> **App 名称**：聚场 (JuChang)
...
### 7.2 Widget_Dashboard 三态设计

Widget_Dashboard 根据用户状态显示不同内容，**确保对话永远不会"死"在空白状态**：

| 状态 | 显示内容 | 用户行动 |
|------|---------|---------|
| **有快捷按钮** | 问候语 + quickActions + 兜底询问 | 一键触发探索/继续草稿/找搭子 |
| **有待参加活动** | 问候语 + 活动列表（最多 3 个） | 点击查看活动详情 |
| **都没有（空状态）** | 问候语 + 引导文案 + 热门 Prompt | 点击 Prompt 直接填充输入框 |

**快捷按钮类型 (quickActions)**：
- `explore_nearby` → 探索附近活动
- `continue_draft` → 继续未发布的草稿
- `find_partner` → 找搭子（预填输入框）
...
| **找搭子** | **谁组我就去/懒得组局/想参加** | **Broker Mode** |
...
| **Broker_Mode** | **高级经纪人模式，帮用户找搭子而非自己组局 (v4.0)** |
| **Partner_Intent** | **搭子意向，用户想参加某类活动的意愿表达 (v4.0)** |
| **Temp_Organizer** | **临时召集人，匹配成功后负责确认并转为活动的用户 (v4.0)** |
| **Widget_AskPreference** | **多轮对话偏好询问卡片，Broker Mode 的结构化追问 (v4.0)** |
