...
## 0.4 我们在解决什么？(The Problem We Solve)
...
| 解法 | 描述 | 用户感受 |
|------|------|----------|
| **变"流"为"桩"** | AI 生成一张精美的**活动邀请函**，丢回群里占屏幕面积大，信息结构化（时间/地点/人数），视觉权重碾压纯文字 | "我的消息终于不会被淹没了" |
...
## 0.5 参与者的 Chat 场景 (The Joiner's Chat)

> **让普通用户也觉得小聚是"我的助理"，而不只是"群主的工具"。**

### 0.5.1 模糊需求的意图理解 (The "I'm Bored" Intent)

不要只等着用户来搜"火锅"。

**用户说**："这周五晚上下班不知道干嘛，不想太累。"

| 传统工具 | 小聚 (Agent) |
|---------|-------------|
| 返回搜索框或列表 | "如果是想放松一下，观音桥那边有个'落日微醺局'还在缺人，或者最近有个新的'滨江路散步摄影'活动。你更倾向哪个？" |

**技术实现**：利用 LLM 的推理能力，把用户的"情绪"映射到活动的"Tag"上。

### 0.5.2 记忆与偏好 (The "Memory" Layer)

> **蚂蚁阿福之所以强，是因为它知道你的账单。小聚要强，必须知道用户的社交偏好。**

**场景**：当用户第二次打开时。

**小聚**："上次你去的那个火锅局感觉怎么样？这周六如果你有空，有个类似的'串串局'，就在你家附近。"

**技术实现**：
- 在 `users` 表或独立的 `user_preferences` 表中，通过 AI 总结用户的历史行为
- 比如：他总是参加周末下午的局，总是喜欢吃辣，总是去观音桥
- RAG (检索增强生成)：把用户的历史活动作为 Context 喂给 AI

### 0.5.3 活动后的"情绪价值" (Post-Activity Engagement)

**场景**：活动结束第二天。

**小聚主动推送**（或用户打开时）："昨晚玩得开心吗？如果是 AA 制的局，需要我帮你算算人均多少钱吗？"

**价值**：这时候小聚就不再是冷冰冰的程序，而是一个贴心的管家。哪怕只是简单的算账功能，通过对话形式展现，温度感完全不同。

### 0.5.4 冷启动策略：没有活动怎么办？

**最怕的场景**：普通用户进来问小聚"附近有啥？"，小聚说"啥也没有。"

**策略一：用"模拟人格"填充空白**

在早期，让小聚显得更"话痨"一点。即使没有活动，用户也可以和小聚聊"重庆哪里好吃"、"最近有什么梗"。

这就把一个"没人用的工具"变成了一个"虽然暂时没局但还挺懂重庆的 AI 聊天机器人"。这能保住用户的留存。

**策略二：引导创建**

```
用户："周末去哪玩？"
小聚："虽然暂时没有群友组局，但我发现解放碑有个新的艺术展很火。要不你来发起一个局？我帮你写文案。"
```

把"无结果"转化为"创建邀请"。这是 Agent 最擅长的引导。

**策略三：POI 热点兜底 (v3.5 新增)**

当附近没有活动时，调用腾讯地图 POI 接口获取热门地点，让对话永远不会"死"在"没有结果"上：

```
用户："观音桥附近有什么好玩的？"
小聚（无活动时）："附近暂时没有群友组局，但我帮你找到了几个最近很火的地方：
- 🍲 朱光玉火锅馆 · 4.8分 · 500m
- 🎮 迷失剧本杀 · 4.6分 · 800m
要不你来发起第一个局？我帮你写文案。"
```

**技术实现**：Widget_Explore 返回空结果时，自动调用 POI 接口填充热门地点，引导用户转化为发起者。

---

## 0.6 全链路体验地图 (Full Journey Map)
...
| 2. 决策 | 查看活动详情 | 显示活动邀请函详情页 | **法务条款入口** (点击报名时需同意隐私协议) |
...
| 组件 | 场景 | 体验优势 |
|------|------|----------|
| **auth-sheet** | 需要手机号时弹出 | 品牌风格半屏，比原生弹窗温柔 |
...
### 7.0 Welcome 页结构 (v4.4 新增)

**核心理念**：
- 用户一进来就看到自己的「社交成就」，有归属感
- 引导完善偏好 → 喂养 `workingMemory`，提升 AI 推荐质量
- 快捷入口降低首次对话门槛
...
**社交档案卡片 (SocialProfileCard)**：

| 元素 | 说明 |
|------|------|
| 折叠标题 | "我的社交档案"，点击展开/收起 |
| 统计数据 | 参与活动数 (`participationCount`) + 发起活动数 (`activitiesCreatedCount`) |
| 引导文案 | "完善偏好，获得更精准推荐" |
| 完善按钮 | 跳转到偏好设置页 (`subpackages/setting/preference/index`) |

**快捷入口 (QuickPrompts)**：

| 入口 | 预填文本 | 触发意图 |
|------|---------|---------|
| 周末附近有什么活动？ | "周末附近有什么活动" | `explore` |
| 帮我找个运动搭子 | "帮我找个运动搭子" | `partner` |
| 想组个周五晚的局 | "想组个周五晚的局" | `create` |

**点击快捷入口**：自动填充到 AI Dock 输入框 → 用户可直接发送或修改
...
| 时间 | 问候语 |
|------|--------|
| 周五 18:00 后 | "Hi～✨ [昵称]，周末到了，想怎么玩？" |
| 周末 | "周末愉快～✨ [昵称]，今天想约啥？" |
| 工作日晚 (18:00-24:00) | "晚上好～✨ [昵称]，今晚有什么安排？" |
| 深夜 (0:00-6:00) | "这么晚还没睡？✨ 想约宵夜还是找人聊天？" |
| 6:00-12:00 | "早上好～✨ [昵称]，新的一天，有什么计划？" |
| 12:00-18:00 | "下午好～✨ [昵称]，下午茶时间，想约个局？" |

### 7.2 Widget_Dashboard 四态设计 (v4.4 更新)
